...
%%   The ``tree'' 
children(ProcState) ->
  case workers:get(ProcState, '$cs418:wtree:children$') of
    [] -> [];
...
reduce_work(ProcState, [{_, RootRight} | CT], LC = {_, Combine}) ->
  ...
  case {Left, Right} of
	...
	throw:Exception ->    failure("error in Combine function for reduce", throw, Exception);
...
reduce_work(ProcState, [], {Leaf, _}) ->
  try
    ...
    throw:Exception -> failure("error in Leaf function for reduce", throw, Exception);
...
reduce_receive(CPid, PS) ->
  receive
    ...
    after workers:get_debug_timeout(PS) ->
      misc:msg_dump("wtree:reduce",
        [io_lib:format("{'$cs418:wtree:reduce$', ~w, V}", [CPid])])
  end.
...
scan_dispatch(CPid, LLC) ->
  PPid = self(),
  CPid ! fun(PS) ->
    {V, Kids} = case children(PS) of
      fail -> {[failure("failure in scan", assertion_failure, "W is not a wtree")], undefined};
      K -> { scan_work1(PS, K, LLC), K}
    end,
    PPid ! {'$cs418:wtree:scan$', self(), hd(V) },
    case Kids of
      undefined -> PS;  % failure, leave worker state unchanged
      _ ->
        Left = scan_receive(PPid, PS),
...
scan_work2(ProcState, [{_, RootRight} | CT], LLC, Left, [LH | LT]) ->
  Combine = element(3, LLC),
  RootRight ! {'$cs418:wtree:scan$', self(), Combine(Left, LH)},
  ...
  receive
    ...
    after workers:get_debug_timeout(PS) ->
      misc:msg_dump("wtree:scan",
        [io_lib:format("{'$cs418:wtree:scan$', ~w, V}", [Pid])])
  end.
