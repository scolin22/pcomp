...
workerProc(ProcState) ->
  receive
    TaskFun when is_function(TaskFun, 1) ->
      try
	NewState =TaskFun(ProcState),
	case {NewState, goodState(NewState)} of
	  {ok, _}   -> workerProc(ProcState);  % ok means 'keep old state'
	  {_, true} -> workerProc(NewState);
	  _ ->
	    io:format("~p: Task function returned invalid process state -- ~p~n", [self(), NewState])
	end
      catch
	throw:Exception ->
...
broadcast(W, Task) when is_function(Task, 2) ->
  [    WW ! fun(ProcState) -> Task(ProcState, ProcIndex) end
...
broadcast(W, Fun, Args) when is_function(Fun, 3) ->
  [    WW ! fun(ProcState) -> Fun(ProcState, ProcIndex, AA) end
...
timeout_val(Seconds) when is_float(Seconds) -> Seconds.
